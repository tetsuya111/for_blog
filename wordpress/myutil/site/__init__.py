...
def __download__(url,output=sys.stdout):
	if not url:
		return bytes()
	resV=_pyio.BytesIO()
	...
	for chunk in requests_get(url,headers=headers):
		resV.write(chunk)
		chunked+=128
...
def download(url,fname,output=sys.stdout,override=False):
	if not override and os.path.exists(fname):
		return False
	output.write("Start donwloading to "+fname+" !!\n")
	res=__download__(url,output)
